...
update_pbp_participation <- function(season) {
  if (season < 2016) {
    ...
    cli::cli_alert_warning(
      "It seems like the {.val {season}} season is gone from the api. Skip to avoid loss of data."
    )
    return(NULL)
  }
  ...
  plays_template <- tibble::tibble(
    ...
    offense_names = character(0),
    defense_names = character(0),
    offense_positions = character(0),
    defense_positions = character(0),
    offense_numbers = character(0),
    defense_numbers = character(0)
  )
